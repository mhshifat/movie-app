...
const ShowInfo: React.FC<Props> = ({ show }) => {
  const [playTrailer, setPlayTrailer] = useState(false);

  return (
    <div className="show-info border-b border-gray-400">
      <div className="container mx-auto px-4 py-16 flex flex-col md:flex-row ">
        ...
        <div className="md:ml-24 ">
          ...
          <div className="flex flex-wrap items-center text-gray-400 text-sm">
            ...
            <span className="ml-1">{show?.vote_average * 10}%</span>
            <span className="mx-2">|</span>
            <span>{show?.first_air_date}</span>
            <span className="mx-2">|</span>
            <span>{show?.genresAsString}</span>
          </div>
          <p className="text-gray-300 mt-8">{show?.overview}</p>
